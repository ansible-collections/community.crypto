...
class KeypairBackend(OpensshModule):
    ...
    def _create_temp_public_key(self, content):
        ...
        try:
            secure_write(temp_public_key, file_mode(self.public_key_path) or 0o644, to_bytes(content))
    ...
    def _restore_public_key(self):
        try:
            self._safe_secure_move([
                (self._create_temp_public_key(str(self._get_public_key()) + '\n'), self.public_key_path)
            ])
        except (IOError, OSError):
...
class KeypairBackendOpensshBin(KeypairBackend):
    ...
    def _get_private_key(self):
        return PrivateKey.from_string(self.ssh_keygen.get_private_key(self.private_key_path)[1])

    def _get_public_key(self):
        return PublicKey.from_string(self.ssh_keygen.get_matching_public_key(self.private_key_path)[1])
...
class KeypairBackendCryptography(KeypairBackend):
    ...
    def _update_comment(self):
        ...
        try:
            self._safe_secure_move([(self._create_temp_public_key(keypair.public_key + b'\n'), self.public_key_path)])
        except (IOError, OSError) as e:
            self.module.fail_json(msg=to_native(e))
