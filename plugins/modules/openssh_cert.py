...
class Certificate(object):
    def __init__(self, module):
        ...
        try:
            self.original_cert_info = OpensshCertificate.load(self.path).cert_info if self.exists() else None
    ...
    def generate(self):
        if not self._is_valid() or self.force:
            if not self.check_mode:
                ...
                try:
                    self.cert_info = OpensshCertificate.load(self.path).cert_info
                except (TypeError, ValueError) as e:
    ...
    def _generate_diff(self):
        before = self.original_cert_info.to_dict() if self.original_cert_info else {}
        before.pop('nonce', None)
        after = self.cert_info.to_dict() if self.cert_info else {}
    ...
    def _is_valid(self):
        if self.original_cert_info:
            try:
                original_time_parameters = OpensshCertificateTimeParameters(
                    valid_from=self.original_cert_info.valid_after,
                    valid_to=self.original_cert_info.valid_before
                )
            except ValueError as e:
                return self.module.fail_json(msg=to_native(e))
            return all([
                self.original_cert_info.cert_type == self.type,
                set(to_text(p) for p in self.original_cert_info.principals) == set(self.principals),
                self.original_cert_info.serial == self.serial_number if self.serial_number is not None else True,
